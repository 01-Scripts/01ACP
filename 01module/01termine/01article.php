...
include_once($subfolder."01acp/system/functions.php");
...
if(isset($_GET[$names['artid']]) && !empty($_GET[$names['artid']]) && $_GET[$names['artid']] != "archiv" && $_GET[$names['artid']] > 0 && is_numeric($_GET[$names['artid']])){
    $query = "SELECT * FROM ".$mysql_tables['artikel']." WHERE frei='1' AND hide='0' AND (endtime >= '".time()."' OR endtime = '0') AND id = '".mysql_real_escape_string($_GET[$names['artid']])."' LIMIT 1";
...
elseif(!empty($settings['archiv_time']) && $settings['archiv_time'] > 0 && is_numeric($settings['archiv_time'])){
    ...
    elseif(isset($_GET[$names['catid']]) && !empty($_GET[$names['catid']]) && substr_count($_GET[$names['catid']], ",") >= 1){
		...
		$query = "SELECT * FROM ".$mysql_tables['artikel']." WHERE frei='1' AND hide='0' AND static='0' AND (endtime >= '".time()."' OR endtime='0') AND timestamp>'".$qt."' AND (".$add2query_cat.") ORDER BY ABS(timestamp-'".time()."')";
		}
	elseif(isset($_GET[$names['catid']]) && !empty($_GET[$names['catid']]))
        $query = "SELECT * FROM ".$mysql_tables['artikel']." WHERE frei='1' AND hide='0' AND static='0' AND (endtime >= '".time()."' OR endtime='0') AND timestamp>'".$qt."' AND newscatid LIKE '%,".mysql_real_escape_string($_GET[$names['catid']]).",%' ORDER BY ABS(timestamp-'".time()."')";	
    else
        $query = "SELECT * FROM ".$mysql_tables['artikel']." WHERE frei='1' AND hide='0' AND static='0' AND (endtime >= '".time()."' OR endtime='0') AND timestamp>'".$qt."' ORDER BY ABS(timestamp-'".time()."')";
...
// Alle Artikel anzeigen (ohne Archiv-Funktion)
    ...
    elseif(isset($_GET[$names['catid']]) && !empty($_GET[$names['catid']]) && substr_count($_GET[$names['catid']], ",") >= 1){
        ...
        $query = "SELECT * FROM ".$mysql_tables['artikel']." WHERE frei='1' AND hide='0' AND static='0' AND (endtime >= '".time()."' OR endtime='0') AND (".$add2query_cat.") ORDER BY ABS(timestamp-'".time()."')";
		}
	elseif(isset($_GET[$names['catid']]) && !empty($_GET[$names['catid']]))
        $query = "SELECT * FROM ".$mysql_tables['artikel']." WHERE frei='1' AND hide='0' AND static='0' AND (endtime >= '".time()."' OR endtime='0') AND newscatid LIKE '%,".mysql_real_escape_string($_GET[$names['catid']]).",%' ORDER BY ABS(timestamp-'".time()."')";
    else
        $query = "SELECT * FROM ".$mysql_tables['artikel']." WHERE frei='1' AND hide='0' AND static='0' AND (endtime >= '".time()."' OR endtime='0') ORDER BY ABS(timestamp-'".time()."')";
...
// NORMALE AUSGABE
	...
	while($row = mysql_fetch_array($list)){
        ...
        if(isset($_GET[$names['artid']]) && !empty($_GET[$names['artid']]) && $_GET[$names['artid']] != "archiv" && $row['static'] == 0){
			...
			$listnext = mysql_query("SELECT id,titel FROM ".$mysql_tables['artikel']." WHERE frei='1' AND hide='0' AND static='0' AND timestamp < '".$row['timestamp']."' AND timestamp > '".(time()-($settings['archiv_time']*24*60*60))."' AND (endtime>='".time()."' OR endtime = '0') ORDER BY ABS(timestamp-'".time()."') DESC LIMIT 1");
            ...
            $listprev = mysql_query("SELECT id,titel FROM ".$mysql_tables['artikel']." WHERE frei='1' AND hide='0' AND static='0' AND timestamp > '".$row['timestamp']."' AND (endtime>='".time()."' OR endtime = '0')ORDER BY ABS(timestamp-'".time()."') LIMIT 1");
...
/* 		if($qt > 0)
		...
		$listcats .= "<option value=\"".$cat['id']."\">".$cat['name']."</option>\n";

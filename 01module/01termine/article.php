...
if(isset($_REQUEST['action']) && ($_REQUEST['action'] == "newarticle" || $_REQUEST['action'] == "newstatic")){
	...
	switch($_REQUEST['action']){
	  case "newarticle":
		$input_field['site_titel'] 	= "Neuer Termin";
		$input_field['bezeichnung'] = "Termin";
		$input_field['next']		= "Neuer Termin";
	...
	if(isset($_POST['do']) && $_POST['do'] == "save" && 
		...
		elseif($saved_id > 0 && $hide == 0 && $frei == 0){
			...
			$emailbody = "Es wurde soeben ein neuer Termin / eine neue Seite erstellt, die von Ihnen überprüft und freigeschaltet werden kann.
...
elseif(isset($_REQUEST['action']) && $_REQUEST['action'] == "edit" && ($userdata['editarticle'] >= 1 || $userdata['staticarticle'] == 1) &&
	...
	switch($_REQUEST['static']){
	  case "0":
		$input_field['site_titel'] 	= "Termin bearbeiten";
		$input_field['bezeichnung'] = "Termin";
		$input_field['next']		= "Neuer Termin";
		$input_section				= "article";
		$input_section2				= "articles";
...
if($flag_overview) $_REQUEST['action'] = $_REQUEST['who'];
	...
	switch($_REQUEST['action']){
	  case "articles":
		$input_field['site_titel'] 	= "Termin-&Uuml;bersicht / Termine bearbeiten";
		$input_field['bezeichnung'] = "Termin";
		$input_section				= "article";

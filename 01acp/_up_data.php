...
elseif(isset($_POST['update']) && $_POST['update'] == "1101_zu_1200"){
	...
	$result = mysql_query($sql_insert) OR die(mysql_error());
	mysql_query("ALTER TABLE `".$mysql_tables['user']."` ADD `01acp_notepad` TEXT NOT NULL default ''");
	//mysql_query("UPDATE `".$mysql_tables['user']."` SET `01acp_notepad` = '1'");
	
	// Diverse MySQL-Tabellenänderungen:
	mysql_query("ALTER TABLE `".$mysql_tables['module']."` CHANGE `serialized_data` `serialized_data` MEDIUMBLOB NULL DEFAULT NULL COMMENT 'use unserialize() to get data back'");
	mysql_query("ALTER TABLE `".$mysql_tables['comments']."` CHANGE `modul` `modul` VARCHAR( 25 ) NULL ,
...
CHANGE `bbc` `bbc` TINYINT( 1 ) NOT NULL DEFAULT '0'");
	mysql_query("ALTER TABLE `".$mysql_tables['filedirs']."` CHANGE `parentid` `parentid` INT( 10 ) NOT NULL DEFAULT '0',
CHANGE `timestamp` `timestamp` INT( 10 ) NOT NULL DEFAULT '0',
CHANGE `name` `name` VARCHAR( 50 ) NULL ,
CHANGE `uid` `uid` INT( 10 ) NOT NULL DEFAULT '0'");
	mysql_query("ALTER TABLE `".$mysql_tables['files']."` CHANGE `type` `type` VARCHAR( 4 ) NULL COMMENT 'Dateityp \"pic\" oder \"file\"',
CHANGE `modul` `modul` VARCHAR( 25 ) NULL");
	mysql_query("ALTER TABLE `".$mysql_tables['menue']."` CHANGE `name` `name` VARCHAR( 50 ) NULL ,
...
CHANGE `subof` `subof` INT( 10 ) NOT NULL DEFAULT '0'");
	mysql_query("ALTER TABLE `".$mysql_tables['module']."` CHANGE `nr` `nr` INT( 3 ) NOT NULL DEFAULT '0',
...
CHANGE `version` `version` VARCHAR( 10 ) NULL");
	mysql_query("ALTER TABLE `".$mysql_tables['rights']."` CHANGE `modul` `modul` VARCHAR( 25 ) NULL ,
...
CHANGE `formwerte` `formwerte` VARCHAR( 255 ) NULL");
	mysql_query("ALTER TABLE `".$mysql_tables['settings']."` CHANGE `modul` `modul` VARCHAR( 25 ) NULL ,
CHANGE `catid` `catid` TINYINT( 2 ) NOT NULL DEFAULT '0',
CHANGE `idname` `idname` VARCHAR( 25 ) NULL ,
CHANGE `formename` `formename` TEXT NULL ,
CHANGE `formwerte` `formwerte` VARCHAR( 255 ) NULL");
	mysql_query("ALTER TABLE `".$mysql_tables['user']."` CHANGE `username` `username` VARCHAR( 50 ) NULL ,
